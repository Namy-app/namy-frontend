...
import { AuthResponse, LoginInput } from "@/lib/api-types";
...
export function useLogin() {
  ...
  return useMutation({
    mutationFn: async ({ rememberMe, ...credentials }: LoginInputWithRememberMe) => {
      const data = await graphqlRequest<SignInResponse>(
        LOGIN_MUTATION,
        { input: credentials }
      );
